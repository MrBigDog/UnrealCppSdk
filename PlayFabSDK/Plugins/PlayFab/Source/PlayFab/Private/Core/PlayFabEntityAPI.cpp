...
bool UPlayFabEntityAPI::ExecuteEntityCloudScript(
    EntityModels::FExecuteEntityCloudScriptRequest& request,
    ...
    const FPlayFabErrorDelegate& ErrorDelegate)
{
    ...
    auto HttpRequest = PlayFabRequestHandler::SendRequest(PlayFabSettings::getURL(TEXT("/CloudScript/ExecuteEntityCloudScript")), request.toJSONString(), TEXT("X-EntityToken"), PlayFabSettings::entityToken);
    HttpRequest->OnProcessRequestComplete().BindRaw(this, &UPlayFabEntityAPI::OnExecuteEntityCloudScriptResult, SuccessDelegate, ErrorDelegate);
    return HttpRequest->ProcessRequest();
}

void UPlayFabEntityAPI::OnExecuteEntityCloudScriptResult(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FExecuteEntityCloudScriptDelegate SuccessDelegate, FPlayFabErrorDelegate ErrorDelegate)
{
    EntityModels::FExecuteCloudScriptResult outResult;
    FPlayFabError errorResult;
    if (PlayFabRequestHandler::DecodeRequest(HttpRequest, HttpResponse, bSucceeded, outResult, errorResult))
    {
        SuccessDelegate.ExecuteIfBound(outResult);
