...
namespace PlayFab
{
    PLAYFABCORE_API class FPlayFabJsonHelpers
    {
    public:

        // Returns the array named Key or nullptr if it is missing or the wrong type
        PLAYFABCORE_API static const TArray< TSharedPtr<class FJsonValue> >& ReadArray(TSharedPtr<class FJsonObject> Item, const FString& Key);
//
...
//        static void WriteValue(JsonWriter& writer, const FString& Identifier, const time_t Value);
        ...
        PLAYFABCORE_API static bool ReadValue(const TSharedPtr<class FJsonObject>& obj, const FString& FieldName, double& OutNumber);
        PLAYFABCORE_API static bool ReadValue(const TSharedPtr<class FJsonObject>& obj, const FString& FieldName, int32& OutNumber);
        PLAYFABCORE_API static bool ReadValue(const TSharedPtr<class FJsonObject>& obj, const FString& FieldName, uint32& OutNumber);
